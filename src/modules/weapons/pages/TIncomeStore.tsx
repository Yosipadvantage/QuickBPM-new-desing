...
import { Button, ButtonGroup, MenuItem, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TablePagination, TableRow, TextField, ThemeProvider, Tooltip } from '@mui/material'
...
interface ITIncomeStore { }

const _weaponService = new WeaponsService();

export const TIncomeStore: React.FC<ITIncomeStore> = (props: ITIncomeStore) => {
    ...
    const getStates = () => {
        setSpinner(true);
        _weaponService.listarEstadosPorEntradaAlmacen().subscribe(resp => {
            console.log(resp);
            if (resp) {
                console.log(resp);
    ...
    const getRecords = () => {
        setSpinner(true);
        _weaponService.getEntradaAlmacenByState(formatDate(dateFrom), formatDate(dateUpto), parseInt(state)).subscribe(resp => {
            setSpinner(false);
            if (resp) {
                console.log(resp);
                    ...
                    <form>
                        <Row className="card box-s m-3 d-block">
                            <Col sm={12} className="mt-5 mb-5 mr-5 ml-5">
                                <h1>..::CONSULTAR REGISTROS DE ENTRADA A ALMACÉN::..</h1>
                            </Col>
                            <Col sm={12}>
                                        ...
                                        {records
                                            ...
                                            .map((item: any) => (
                                                <TableRow hover role="checkbox" tabIndex={-1}>
                                                    <TableCell>{item.DataBeanProperties.IDEntradaAlmacen}</TableCell>
                                                    <TableCell>{item.DataBeanProperties.NoEntradaRef}</TableCell>
